...
public class HttpUtil
{
	...
	private static String http_post(AppContext appContext, String url, Map<String, String> params) throws AppException{
		String cookie = getCookie(appContext);
		HttpClient httpClient = null;
	 ...
	 * @throws AppException
	 */
	private static HttpEntity http_post(AppContext appContext, String url, Map<String, String> params, Map<String,File> files) throws AppException {
        ...
        {
        	for(String key:params.keySet()){
        		multipartEntityBuilder.addTextBody(key,  params.get(key), ContentType.create("text/plain", Charset.forName(UTF_8)));
        	}
        }
	...
	public static List<XContent> getContentData(AppContext appContext,int type,int page){
		...
		try
		{
			...
			Gson gson = new Gson();
			xConList=gson.fromJson(result,  new TypeToken<List<XContent>>() {}.getType());
	...
	public static XContentDetail getContentDetail(AppContext appContext,String url){
		XContentDetail xcon=null;
		try
		{
			String result=http_get(appContext, url);
			Gson gson = new Gson();
			xcon=gson.fromJson(result, XContentDetail.class);
		} catch (AppException e)
		{
			e.printStackTrace();
		}
	 ...
	 * @throws AppException
	 */
	public static  XComment pubConmment(AppContext appContext, long userId, long contentId,String commentInfo,long replyId) throws AppException {
		Map<String,String> params = new HashMap<String,String>();
	 ...
	 * @param userId
	 * @param contentId
	 * @param isCancel 是否取消赞
	 * @throws AppException 
	 */
	public static XPraise praiseContent(AppContext appContext,long userId,long contentId,boolean isCancel) throws AppException{
		Map<String,String> params = new HashMap<String,String>();
		params.put("contentid", contentId+"");
		params.put("userId", userId+"");
	...
	public static List<XGoodType> getMarketType(AppContext appContext)throws AppException{
		...
		try
		{
			...
			System.out.println("json---"+result);
			Gson gson = new Gson();
			List<XGoodType> xtypesList=gson.fromJson(result,  new TypeToken<List<XGoodType>>() {}.getType());
			return xtypesList ;
		} catch (AppException e)
		{
			e.printStackTrace();
